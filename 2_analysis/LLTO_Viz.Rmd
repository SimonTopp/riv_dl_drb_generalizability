---
title: "LLLTO_Viz"
output: html_document
editor_options: 
  chunk_output_type: console
---

```{r setup, include=FALSE}
library(tidyverse)
library(lubridate)
library(reticulate)
library(sf)
library(plotly)
library(feather)
source('utils.R')
library(ggridges)


knitr::opts_chunk$set(echo = TRUE)
```

## Baseline Comparisons

```{r}
spatial <- readRDS('../data/in/DRB_spatial/network.rds')
edges <- spatial$edges %>% st_as_sf()

rgcn_stats <- summary_stats('../data/rgcn_out/output_DRB_baseline', 'rgcn')[[1]]
# rgcn_stats <- map2(rgcn_stats[[1]], rgcn_stats[[2]], bind_rows)
# 
# rgcn_stats$temps %>% select(-c(count, bias)) %>% pivot_wider(names_from=run, values_from = rmse) %>%
#   mutate(diff = baseline-no_pretrain)

gwn_stats <- summary_stats('../data/out/hypertuning_kernel_layer/60_15', 'gwn')[[2]]


## Figure for overall difference between the two models

p1 <- gwn_stats$segs %>% mutate(run = 'GraphWaveNet') %>% bind_rows(rgcn_stats$segs %>% mutate(run = 'RGCN')) %>%
  filter(count > 10) %>%
  seg_plotter_sf(., 'rmse') +
  facet_wrap(~run)

baseline_diff <- seg_rmse_diff(rgcn_stats$segs, gwn_stats$segs, 'RGCN', 'GraphWaveNet')

p2 <- baseline_diff %>% seg_plotter_sf(., 'rmse_diff') +
  scale_color_gradient2(na.value = 'white', mid = 'grey90', name = 'RMSE\nDifference') +
  labs(title = 'RGCN minus GWN') +
  theme(title = element_text(size =8))


p3 <- rgcn_stats$months %>% mutate(model= 'RGCN') %>%
  bind_rows(gwn_stats$months %>% mutate(model = 'GraphWaveNet')) %>%
  pivot_longer(c(rmse, bias)) %>%
  mutate(name = factor(name, levels = c('rmse', 'bias'))) %>%
  ggplot(aes(x=month, y=value, fill=model, shape = name)) +
  scale_fill_viridis_d(end =.6) +
  #scale_y_continuous(breaks = c(0,1,2)) +
  geom_col(position = 'dodge') +
  facet_wrap(~name,nrow=2, scales='free_y') +
  theme_minimal() +
  labs(x = 'Month', y = '°Celsius')


layout <- rbind(c(1,1,2),
                c(1,1,2),
                c(1,1,2),
                c(3,3,3),
                c(3,3,3))

g <- gridExtra::grid.arrange(p1,p2,p3, layout_matrix = layout)

#ggsave('figures/baseline_comps.png',plot = g, width = 5, height = 4.5, units = 'in')

rgcn_stats$temps %>% mutate(model = 'RGCN') %>%
  bind_rows(gwn_stats$temps %>% mutate(model = 'GWN')) %>%
  select(temp_group, rmse, model) %>%
  filter(temp_group != 'Normal Temp') %>%
  mutate(rmse = round(rmse, 2)) %>%
  rename(`Temp. Bin` = temp_group) %>%
  pivot_wider(names_from = model, values_from = rmse) %>%
  relocate(RGCN, .after = GWN) %>%
  kableExtra::kbl(caption = 'Overall RMSE by Temperature Bin') %>%
  kableExtra::kable_material(full_width = F)


```

### LTO Comps

```{r}
rgcn_lto <- summary_stats('../data/rgcn_out/lto', 'rgcn')
rgcn_lto <- map2(rgcn_lto[[1]], rgcn_lto[[2]], bind_rows)

gwn_lto <- summary_stats('../data/out/lto', 'gwn')
gwn_lto <- map2(gwn_lto[[1]], gwn_lto[[2]], bind_rows)

rgcn_diff_min <- seg_rmse_diff(rgcn_stats$segs, rgcn_lto$segs %>% filter(run == 'lto_min'), 'Baseline', 'Holdout')
rgcn_diff_max <- seg_rmse_diff(rgcn_stats$segs, rgcn_lto$segs %>% filter(run == 'lto_max'), 'Baseline', 'Holdout')

gwn_diff_min <- seg_rmse_diff(gwn_stats$segs, gwn_lto$segs %>% filter(run == 'lto_min'), 'Baseline', 'Holdout')
gwn_diff_max <- seg_rmse_diff(gwn_stats$segs, gwn_lto$segs %>% filter(run == 'lto_max'), 'Baseline', 'Holdout')

full <- rgcn_diff_max %>% mutate(model = 'RGCN', holdout = 'Max Temps') %>%
  bind_rows(rgcn_diff_min%>% mutate(model = 'RGCN', holdout = 'Min Temps')) %>%
  bind_rows(gwn_diff_max  %>% mutate(model = 'GWN', holdout = 'Max Temps')) %>%
  bind_rows(gwn_diff_min  %>% mutate(model = 'GWN', holdout = 'Min Temps'))

sig_test <- full %>%
  filter(!is.na(rmse_diff)) %>%
  group_by(holdout, model) %>%
  nest() %>%
  mutate(wilcox = map(data, ~wilcox.test(.$Baseline, .$Holdout) %>% broom::tidy())) %>%
  select(-data) %>%
  unnest(cols = c(wilcox)) %>%
  mutate(p.value = round(p.value,3))


full %>% 
  select(seg_id_nat, rmse_diff) %>%
  mutate(rmse_diff = ifelse(rmse_diff > 2, 2, ifelse(rmse_diff < -2, -2, rmse_diff))) %>%
  seg_plotter_sf(., 'rmse_diff') +
  scale_color_gradient2(na.value = 'white', mid = 'grey80', name = 'RMSE\nDifference', labels = c('<-2', '-1', "0", '1', '>2')) +
  facet_grid(holdout~model) 
  #ggtitle('Baseline Minus Temperature Holdut Runs')

ggsave('figures/lto_spatial.png', width = 4, height = 5, units = 'in')

lto_full <- gwn_lto$segs %>%
  bind_rows(gwn_stats$segs %>% mutate(run = 'baseline')) %>% mutate(model = 'GWN') %>%
  bind_rows(rgcn_lto$segs %>% bind_rows(rgcn_stats$segs) %>% mutate(model = 'RGCN'))

meds <- lto_full %>%  
  group_by(model, run) %>%
  summarise(median = median(rmse))

ggplot(lto_full, aes(x = rmse, y = run, fill = model)) + geom_density_ridges(alpha = .3, scale = .9) +
  scale_fill_viridis_d(end = .6) + 
  geom_vline(data= meds %>% filter(run == 'baseline'), aes(xintercept = median, color = model)) +
  geom_point(data= meds, aes(x = median, color = model), size = 2, shape =23) +
  scale_color_viridis_d(end = .6) + 
  scale_y_discrete(labels = c('Baseline', 'Train Cold/\nTest Hot', 'Train Hot/\nTest Cold')) +
  xlim(0,7) +
  theme_classic() +
  ggtitle('Comparison of Reach Scale RMSE')

ggsave('figures/lto_ridges.png', width = 4.5, height = 3, units = 'in')  

rgcn_stats$temps %>% bind_rows(rgcn_lto$temps) %>% 
  mutate(model = 'RGCN') %>%
  bind_rows(gwn_stats$temps %>% mutate(run = 'baseline') %>% bind_rows(gwn_lto$temps) %>%
              mutate(model = 'GWN')) %>%
  select(temp_group, run, rmse, model) %>%
  filter(temp_group != 'Normal Temp') %>%
  mutate(rmse = round(rmse, 2)) %>%
  rename(`Temp. Bin` = temp_group) %>%
  pivot_wider(names_from = run, values_from = rmse) %>%
  mutate(`Max Dif` = baseline-lto_max,
         `Min Dif` = baseline-lto_min) %>%
  select(`Temp. Bin`, `Train Cold/Test Hot` = `Max Dif`, `Train Hot/Test Cold` = `Min Dif`) %>%
  kableExtra::kbl(caption = 'Overall Change in RMSE from Baseline by Temperature Bin') %>%
  kableExtra::kable_material(full_width = F)

```


### LLO Comps


```{r}
llo_groups <- read_csv('data/in/DRB_spatial/llo_groups.csv')

rgcn_llo <- summary_stats('../data/rgcn_out/llo', 'rgcn', llo = T)
rgcn_llo <- pmap(rgcn_llo, bind_rows)

gwn_llo <- summary_stats('../data/out/llo', 'gwn', llo =T)
gwn_llo <- pmap(gwn_llo, bind_rows)

p1 <- rgcn_llo$segs %>% mutate(model = 'RGCN') %>%
  bind_rows(gwn_llo$segs %>% mutate(model = 'GWN')) %>%
  seg_plotter_sf(., 'rmse') +
  facet_wrap(~model, nrow =1)

rgcn_diff <- seg_rmse_diff(rgcn_stats$segs, rgcn_llo$segs, 'Baseline', 'Holdout')

gwn_diff <- seg_rmse_diff(gwn_stats$segs, gwn_llo$segs, 'Baseline', 'Holdout')

full <- gwn_diff %>% select(seg_id_nat, rmse_diff) %>% mutate(model = 'GWN') %>%
  bind_rows(rgcn_diff %>% select(seg_id_nat, rmse_diff) %>% mutate(model = 'RGCN'))

p2 <-full %>% 
  mutate(rmse_diff = ifelse(rmse_diff > 3, 3, ifelse(rmse_diff < -3, -3, rmse_diff))) %>%
  seg_plotter_sf(., 'rmse_diff') +
  scale_color_gradient2(na.value = 'white', mid = 'grey80', name = 'RMSE\nDifference', breaks = c(-3,-1.5,0,1.5,3),labels = c('<-3', '-1.5', "0", '1.5', '>3')) +
  facet_wrap(~model) 

g <- gridExtra::grid.arrange(p1,p2, nrow =2)

ggsave('figures/llo_spatial.png',plot=g, width = 4,height = 6, units = 'in')  

rgcn_base_llo <- rgcn_stats$segs %>% mutate(test_group = case_when(seg_id_nat %in% llo_groups$seg_id_nat[llo_groups$test_group == 1]~'group_1',
                                                                   seg_id_nat %in% llo_groups$seg_id_nat[llo_groups$test_group == 2]~'group_2',
                                                                   seg_id_nat %in% llo_groups$seg_id_nat[llo_groups$test_group == 3]~'group_3'),
                                            model = 'RGCN') %>%
  select(seg_id_nat, rmse, test_group, model)

gwn_base_llo <- gwn_stats$segs %>% mutate(test_group = case_when(seg_id_nat %in% llo_groups$seg_id_nat[llo_groups$test_group == 1]~'group_1',
                                                                   seg_id_nat %in% llo_groups$seg_id_nat[llo_groups$test_group == 2]~'group_2',
                                                                   seg_id_nat %in% llo_groups$seg_id_nat[llo_groups$test_group == 3]~'group_3'),
                                          model = 'GWN') %>%
  select(seg_id_nat, rmse, test_group, model)

full_reshape <- rgcn_diff %>% mutate(model = 'RGCN') %>%
  bind_rows(gwn_diff %>% mutate(model = 'GWN')) %>%
  mutate(test_group = case_when(seg_id_nat %in% llo_groups$seg_id_nat[llo_groups$test_group == 1]~'Headwaters',
                                seg_id_nat %in% llo_groups$seg_id_nat[llo_groups$test_group == 2]~'Piedmont',
                                seg_id_nat %in% llo_groups$seg_id_nat[llo_groups$test_group == 3]~'Coastal'))


full_reshape %>%
  group_by(model) %>%
  summarise(mean_diff = round(mean(rmse_diff, na.rm = T),3),
            sd_dff = round(sd(rmse_diff, na.rm = T),3),
            median = round(median(rmse_diff, na.rm = T),3))

sig_test <- full_reshape %>%
  filter(!is.na(rmse_diff)) %>%
  select(-c(Baseline, Holdout)) %>%
  pivot_wider(names_from = model, values_from = rmse_diff) %>%
  group_by(test_group) %>%
  nest() %>%
  mutate(wilcox = map(data, ~wilcox.test(.$GWN, .$RGCN) %>% broom::tidy()))

sig_test <- full_reshape %>%
  filter(!is.na(rmse_diff)) %>%
  #select(-c(Baseline, Holdout)) %>%
  #pivot_wider(names_from = model, values_from = rmse_diff) %>%
  group_by(test_group, model) %>%
  nest() %>%
  mutate(wilcox = map(data, ~wilcox.test(.$Baseline, .$Holdout) %>% broom::tidy())) %>%
  select(-data) %>%
  unnest(cols = c(wilcox)) %>%
  mutate(p.value = round(p.value,3))


full_reshape %>% filter(!is.na(rmse_diff)) %>%
  select(-rmse_diff) %>%
  pivot_longer(c(Baseline, Holdout)) %>%
  mutate(test_group = factor(test_group, levels = c('Coastal', 'Piedmont','Headwaters'))) %>%
  ggplot(., aes(x = value, y = test_group, fill = name)) +
    geom_density_ridges(
    quantile_lines = TRUE, quantiles = .5, scale = 0.9, alpha = 0.7,
    vline_size = 1) +
  scale_fill_viridis_d(alpha = .3, end = .6) +
  facet_wrap(~model) +
  coord_cartesian(xlim=c(0,8)) +
  labs(x = 'RMSE', y='Holdout Region', fill = 'Model Run') +
  theme_bw()

ggsave('figures/llo_dist.png',width = 5.5, height = 4, units = 'in')
```


## Make some plots that show change in segment RMSE as 


```{r pressure, echo=FALSE}

full_log <- llo_rmse %>% full_join(edges %>% st_set_geometry(NULL)) %>% mutate(comp = 'Leave Location Out') %>%
  bind_rows(baseline %>% full_join(edges %>% st_set_geometry(NULL)) %>% mutate(comp = 'Baseline')) 

p1 <- full_log %>%
  seg_plotter_sf(., 'rmse') + 
  facet_wrap(~comp) + 
  ggtitle('RMSE')

check <- full_log %>% 
  select(comp,seg_id_nat, rmse) %>% 
  filter(!is.na(seg_id_nat)) %>% 
  pivot_wider(names_from=comp, values_from = rmse) %>%
  mutate(rmse_diff = Baseline-`Leave Location Out`) %>%
  left_join(full_log %>% select(run, seg_id_nat) %>% filter(run != 'baseline'))

p2 <- check %>% mutate(rmse_diff = case_when(rmse_diff > 5 ~ 5,
                                       rmse_diff < -5 ~ -5,
                                       is.numeric(rmse_diff)~rmse_diff)) %>%
  seg_plotter_sf(., 'rmse_diff') +
  scale_color_gradient2(na.value = 'white', mid = 'grey90') +
  ggtitle('RMSE Difference \n(Baseline - LLO)')

medians <- full_log %>% group_by(comp) %>%
  summarise(med = median(rmse, na.rm = T))

p3 <- ggplot(full_log, aes(x = rmse)) + 
  geom_density(fill = 'grey70') +
  geom_vline(data = medians, aes(xintercept =med), color = 'red') +
  facet_wrap(~comp, nrow = 1, scales = 'free_x') +
  theme_minimal()

p4 <- ggplot(check, aes(x = rmse_diff, fill = run)) + 
  geom_density(alpha = .7) +
  geom_vline(aes(xintercept =median(rmse_diff, na.rm=T)), color = 'red') +
  scale_fill_viridis_d(labels = c('Headwaters', 'Piedmont', 'Coastal'))+
  labs(fill = 'Test Group', x = 'rmse difference') +
  theme_minimal()

layout <- rbind(c(1,1,1,2,2),c(1,1,1,2,2),c(3,3,4,4,4))
gridExtra::grid.arrange(p1,p2, p3,p4, layout_matrix = layout) #nrow = 2, widths = c(.66,.33), heights = c(.66,.33))

```



##### New output format figs

```{r}
summary_results <- function(folder, id){
    files <- list.files(folder, full.names = T) 
    segs <- read_csv(grep('reach', files, value = T))%>% mutate(run = id)
    months <- read_csv(grep('month',files, value=T))%>% mutate(run = id)
    temps <- read_csv(grep('overall',files,value=T))%>% mutate(run = id)
    
    return(mget(c('segs', 'months','temps')))
}

llo1 <- summary_results('data/out/llo/LLO1/', 'group_1')
llo2 <- summary_results('data/out/llo/LLO2/', 'group_2')
llo3 <- summary_results('data/out/llo/LLO3/', 'group_3')
baseline <- summary_results('data/out/repeat_2/','baseline')


llo <- list(llo1,llo2,llo3) %>% pmap(bind_rows)
seg_plotter_sf(llo$segs %>% filter(!is.na(piw), partition == 'test'), 'rmse')# +
  scale_color_viridis_c(option = 'inferno', direction = -1, limits= c(0,18))

llo$segs %>% filter(!is.na(picp), partition == 'test') %>% summarise(mean = median(picp))
```

## Put together some utilities to comine replicate runs into summary outputs
```{r}
combine_replicates = function(folder, pattern){
  files <- list.files(folder, pattern='overall_metrics', recursive=T, full.names=T)
  
  for(i in c(1:length(files))){
    if(i == 1){
      df <- read_csv(files[i]) %>% mutate(replicate = i)
    }
    else{df <- df %>% bind_rows(read_csv(files[i]) %>% mutate(replicate = i))
    }
  }
  
   df <- df %>% group_by(partition, date, seg_id_nat, replicate) %>%
     summarise()
  
  outputs_list <- files %>% map(~read_csv(.) %>% select(observed, predicted, ci_low, ci_high) %>% mutate())
  
  means <- outputs_list %>% pmap(reduce, mean, na.rm=T)
}


ggplot(df, aes(x=nse)) + geom_histogram() + 
  geom_vline(aes(xintercept = mean(nse))) +
  facet_wrap(~group)

df %>% group_by(group, partition) %>%
  select(-replicate) %>%
  summarise(across(everything(),list(mean = mean, sd = sd),.names= "{.col}.{fn}"))
```

