...
gwn_reps <- read_replicates('results/baseline/GWN', 'overall_metrics') ## Best, 9
rgcn_reps <- read_replicates('results/baseline/RGCN', 'overall_metrics') ## Best, 7
rgcn_hot_reps <- read_replicates('results/LTO/GWN/max', 'overall_metrics')

shapiro.test(rgcn_reps$rmse[rgcn_reps$partition=='tst'])
shapiro.test(gwn_reps$rmse[gwn_reps$partition=='tst'])
...
p3 <- check <- rgcn_stats$months %>% filter(partition == 'tst') %>% mutate(model= 'RGCN') %>%
